...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$admin = $this->db->get_where('smaga_user', ['email' => $user])->row_array();
			if ($admin) {
				$validate_by = 'email';
			} else {
				$validate_by = 'user';
			}
			$this->_login($user, $pass, $validate_by);
		}
	}

	private function _login($user, $pass, $validate_by)
	{
		if ($validate_by == 'email') {
			// jika validasi lwat email
			$admin = $this->db->get_where('smaga_user', ['email' => $user])->row_array();
		} else {
			// jika validasi lewat username
			$admin = $this->db->get_where('smaga_user', ['username' => $user])->row_array();
		}
		if ($admin) {
			if (password_verify($pass, $admin['password'])) {
				if ($admin['is_active'] == 1) {
					...
					// $_SESSION['ses_kcfinder']['uploadURL'] = base_url().'assets/js/content_upload';
					...
					//user belum di verifikasi
					$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">User Belum di Verifikasi!</div>');
	...
	public function daftar()
	{
		...
		$this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email|is_unique[smaga_user.email]', [
		...
		if ($this->form_validation->run() == false) {
			...
			$this->db->insert('smaga_user_token', $user_token);
	...
	public function verify()
	{
		...
		if ($user) {
			$user_token = $this->db->get_where('smaga_user_token', ['token' => $token])->row_array();
			if ($user_token) {
				if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
					$this->db->set('is_active', 1);
					$this->db->where('email', $email);
					$this->db->update('smaga_user');
					$this->db->delete('smaga_user_token', ['email' => $email]);
					...
					$this->db->delete('smaga_user', ['email' => $email]);
					$this->db->delete('smaga_user_token', ['email' => $email]);
	...
	public function daftaradmin()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->db->insert('smaga_user_admin', $data);
	...
	public function gantipassword()
	{
		...
		$role = $this->db->get_where('smaga_user_role', ['id' => $role_id])->row_array();
		$user = $this->db->get_where('smaga_user', ['email' => $email])->row_array();
		$dbuser = 'smaga_user';
	...
	public function forgotPassword()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$user = $this->db->get_where('smaga_user', ['email' => $email, 'is_active' => 1])->row_array();
			if ($user) {
				$token = substr(md5(mt_rand()), 0, 32);
				$user_token = [
					'email' => $email,
