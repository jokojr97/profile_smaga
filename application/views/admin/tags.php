        ...
        <div class="row" >
          <div class="col-md-12">
            <div class="table table-responsive">
              <a href="#" class="btn btn-primary" onclick="tambahfunction()"><span class="fas fa-plus"></span> Tambah Berita</a>
              <br><br>
...
function hapusfunction(id) {
    var con = confirm('Apakah Anda yakin untuk menghapus tag ini?');
    if (con == true) {  
        // Simulate an HTTP redirect:
        var url = '<?= base_url() ?>admin/tags/hapus/' + id;
...
function tambahfunction() {
    var inptprompt = prompt('Masukkan Nama tag:');
    if (inptprompt) {
        $.ajax({
            url: "<?= base_url() ?>admin/tags/insert",
            type: "GET",
            action: "insert",
            data: {tags: inptprompt},
...
function editfunction(id) {
    // console.log(id);
    $.ajax({
        url: "<?= base_url() ?>admin/tags/get_name",
        type: "GET",
        action: "get_name",
        data: {id: id},
        success: function (response) {
            res = JSON.parse(response);
            var promptedit = prompt('Nama tag', res.name);
            if (promptedit) {
